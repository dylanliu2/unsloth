...
def Gemma2DecoderLayer_fast_forward(
    ...
    *args, **kwargs,
    ...
    if use_cache and hasattr(self, "_flag_for_generation"): #past_key_value is not None:
        out_weight = torch.empty(self.input_layernorm.weight.shape, dtype = torch.float32, device = hidden_states.device)
...
def Gemma2Attention_fast_forward_inference(
    ...
    RH_K = RH_Q[:,:n_kv_heads,:,:] # torch.empty((n_kv_heads, 1, head_dim), dtype = dtype, device = hidden_states.device)
    RH_K[:,:,:,:h] = Kn[:,:,:,h:]
    RH_K[:,:,:,h:] = Kn[:,:,:,:h]
...
def Gemma2Model_fast_forward_inference(
    ...
    out_weight = torch.empty_like(self.model.layers[0].input_layernorm.weight, dtype = torch.float32, device = input_ids.device)
